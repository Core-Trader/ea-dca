...
//| Taken right after this session's source-code audit fixes (A1–A5)   |
//| and the correct, guide-derived touch-mode breach-latch mechanism   |
//| for BB Centre Band, but BEFORE the docs-audit fixes §C1 and §H1.   |
//| Relative to the current live DCA_EA.mq5, this snapshot is missing: |
//| §C1 (Recovery Mode's breakeven check was unconditionally the full  |
//| breakeven+buffer, even on the very first evaluation — the guide's  |
//| own text describes two tiers, breakeven-or-better first, buffer    |
//| only once already in Recovery Mode) and §H1 (the entry zone-breach |
//| latch required touch AND close-beyond-the-band, not just a touch,  |
//| in both BBBuyBreach()/BBSellBreach() and the OnInit() historical   |
//| lookback scan). Kept as the "before either docs-audit exit/entry   |
//| fix" checkpoint, since testing them individually and combined       |
//| produced genuinely mixed results worth being able to reproduce.    |
//|                                                                    |
...
//+------------------------------------------------------------------+
//| ARCHIVED PRE-C1/H1 SNAPSHOT (DCA_EA_V2.mq5) — this is the original, |
//| stricter definition: touch AND close beyond the band. §H1 (touch    |
//| alone) is the live code's current behavior as of the archive date.  |
//+------------------------------------------------------------------+
...
void HandleBBCentreOrQQE50(bool isBuy, int idx, bool isBBMode)
  {
   ...
   if(!InpUseDynamicStop)
     {
      //--- ARCHIVED PRE-C1 SNAPSHOT: unconditional buffer threshold (§C1 not yet applied).
