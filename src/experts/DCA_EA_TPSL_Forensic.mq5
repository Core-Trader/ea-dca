//+------------------------------------------------------------------+
//|                                          DCA_EA_TPSL_Forensic.mq5 |
//|          TP/SL Mode Diagnostic Build — MFE/MAE Root-Cause Audit   |
//|                                                                    |
//| Exact copy of DCA_EA.mq5 (diff-verified, pure-read logging only,   |
//| same convention as DCA_EA_Forensic.mq5 earlier in this project).   |
//| Adds real-time MFE/MAE tracking for every TP/SL-mode position,      |
//| keyed off actual broker positions (PositionGetTicket()/magic/       |
//| symbol), not the internal Sequence state — this measures real       |
//| price excursion the same way a trader watching the position would,  |
//| independent of any assumption about our own bookkeeping being       |
//| correct. Logs one CSV row per closed TP/SL position with entry,     |
//| exit, direction, MFE, MAE, profit captured vs. MFE given back, exit |
//| reason (from the broker's own deal comment), and duration. Never    |
//| alters a trading decision — see ForensicUpdate()/OnTick() below.    |
//+------------------------------------------------------------------+
...
//+====================================================================+
//| FORENSIC INSTRUMENTATION (this diagnostic build only — never in     |
//| DCA_EA.mq5/EA_DCA_CENT_V1.mq5). Tracks real-time MFE/MAE for every   |
//| TP/SL-mode position, keyed off the actual broker position (ticket,  |
//| magic, symbol) rather than our own Sequence bookkeeping — this is   |
//| deliberate: it measures what really happened to the position's      |
//| price, independent of whether our internal state is itself correct, |
//| so it can't mask a bug in the code it's meant to help diagnose.      |
//+====================================================================+
ulong    g_fxTicket[];
double   g_fxEntryPrice[];
bool     g_fxIsBuy[];
double   g_fxLot[];
double   g_fxMFEPips[];    // best (most favorable) excursion ever seen, in pips — starts at 0
double   g_fxMAEPips[];    // worst (most adverse) excursion ever seen, in pips — negative, starts at 0
datetime g_fxOpenTime[];
int      g_fxFileHandle = INVALID_HANDLE;

void ForensicInit()
  {
   string fname = "tpsl_forensic_" + IntegerToString(InpTradeMode) + "_" +
                  IntegerToString((int)InpSLType) + "_" + IntegerToString((int)InpExitStrategy) + ".csv";
   g_fxFileHandle = FileOpen(fname, FILE_WRITE | FILE_CSV | FILE_ANSI, ',');
   if(g_fxFileHandle == INVALID_HANDLE)
     {
      Print("FORENSIC: failed to open output file, error ", GetLastError());
      return;
     }
   FileWrite(g_fxFileHandle, "ticket", "direction", "open_time", "entry", "exit_time", "exit",
             "lot", "realized_profit", "mfe_pips", "mae_pips", "duration_hours", "exit_comment");
  }

int ForensicFindOrCreate(ulong ticket, double entry, bool isBuy, double lot, datetime openTime)
  {
   int n = ArraySize(g_fxTicket);
   for(int i = 0; i < n; i++)
      if(g_fxTicket[i] == ticket) return(i);

   ArrayResize(g_fxTicket, n + 1);
   ArrayResize(g_fxEntryPrice, n + 1);
   ArrayResize(g_fxIsBuy, n + 1);
   ArrayResize(g_fxLot, n + 1);
   ArrayResize(g_fxMFEPips, n + 1);
   ArrayResize(g_fxMAEPips, n + 1);
   ArrayResize(g_fxOpenTime, n + 1);
   g_fxTicket[n] = ticket; g_fxEntryPrice[n] = entry; g_fxIsBuy[n] = isBuy; g_fxLot[n] = lot;
   g_fxMFEPips[n] = 0.0; g_fxMAEPips[n] = 0.0; g_fxOpenTime[n] = openTime;
   return(n);
  }

void ForensicRemoveAt(int i)
  {
   int n = ArraySize(g_fxTicket);
   for(int k = i; k < n - 1; k++)
     {
      g_fxTicket[k] = g_fxTicket[k+1]; g_fxEntryPrice[k] = g_fxEntryPrice[k+1];
      g_fxIsBuy[k] = g_fxIsBuy[k+1]; g_fxLot[k] = g_fxLot[k+1];
      g_fxMFEPips[k] = g_fxMFEPips[k+1]; g_fxMAEPips[k] = g_fxMAEPips[k+1];
      g_fxOpenTime[k] = g_fxOpenTime[k+1];
     }
   ArrayResize(g_fxTicket, n - 1); ArrayResize(g_fxEntryPrice, n - 1);
   ArrayResize(g_fxIsBuy, n - 1); ArrayResize(g_fxLot, n - 1);
   ArrayResize(g_fxMFEPips, n - 1); ArrayResize(g_fxMAEPips, n - 1);
   ArrayResize(g_fxOpenTime, n - 1);
  }

//+------------------------------------------------------------------+
//| Called every tick, TP/SL mode only, read-only with respect to any  |
//| trading decision. Step 1: update MFE/MAE for every open position    |
//| belonging to this EA. Step 2: any ticket we were tracking that is    |
//| no longer an open position has closed — pull its actual exit from   |
//| history and log the full record.                                    |
//+------------------------------------------------------------------+
void ForensicUpdate()
  {
   if(InpTradeMode != MODE_TPSL || g_fxFileHandle == INVALID_HANDLE) return;
   ...
   for(int i = 0; i < PositionsTotal(); i++)
     {
      ...
      double current = isBuy ? SymbolInfoDouble(_Symbol, SYMBOL_BID) : SymbolInfoDouble(_Symbol, SYMBOL_ASK);
      double pips = (isBuy ? (current - entry) : (entry - current)) / g_pipSize;

      int idx = ForensicFindOrCreate(ticket, entry, isBuy, lot, openTime);
      if(pips > g_fxMFEPips[idx]) g_fxMFEPips[idx] = pips;
      if(pips < g_fxMAEPips[idx]) g_fxMAEPips[idx] = pips;
     }

   for(int i = ArraySize(g_fxTicket) - 1; i >= 0; i--)
     {
      if(PositionSelectByTicket(g_fxTicket[i])) continue;   // still open

      //--- closed: find its own last deal (the close) for exit price/profit/comment.
      double exitPrice = 0.0, profit = 0.0; datetime exitTime = 0; string comment = "";
      if(HistorySelectByPosition(g_fxTicket[i]))
        {
      ...
      double durationHours = (exitTime > 0 && g_fxOpenTime[i] > 0) ? (double)(exitTime - g_fxOpenTime[i]) / 3600.0 : 0.0;
      FileWrite(g_fxFileHandle, g_fxTicket[i], g_fxIsBuy[i] ? "buy" : "sell",
                TimeToString(g_fxOpenTime[i], TIME_DATE|TIME_SECONDS), DoubleToString(g_fxEntryPrice[i], _Digits),
                TimeToString(exitTime, TIME_DATE|TIME_SECONDS), DoubleToString(exitPrice, _Digits),
                DoubleToString(g_fxLot[i], 2), DoubleToString(profit, 2),
                DoubleToString(g_fxMFEPips[i], 2), DoubleToString(g_fxMAEPips[i], 2),
                DoubleToString(durationHours, 2), comment);
      FileFlush(g_fxFileHandle);
      ForensicRemoveAt(i);
     }
  }
...
void OnDeinit(const int reason)
  {
   if(g_fxFileHandle != INVALID_HANDLE) FileClose(g_fxFileHandle);
   SaveState();
...
void OnTick()
  {
   ...
   UpdateChartDisplay();
   SaveStateThrottled();
   ForensicUpdate();   // read-only, never alters a trading decision — see its own header
  }
