//+------------------------------------------------------------------+
//|                                      EA_DCA_CENT_V1_EquityForensic.mq5 |
//|   Forensic diagnostic build — EA_DCA_CENT_V1.mq5 + per-bar equity |
//|   logging only. Roboforex symbol-screening correlation study,     |
//|   SYMBOL_SCREENING_REPORT.md §C/§D item 1: the .htm deal log only  |
//|   gives Balance at each trade CLOSE, which this project has        |
//|   repeatedly found (TPSL_EQUITY_BALANCE_ROOT_CAUSE.md) is near-     |
//|   invisible for this strategy — real risk lives in floating         |
//|   EQUITY while a sequence is still open. This build adds a single,  |
//|   pure-read CSV log of (time, balance, equity, floating profit)     |
//|   once per closed bar, so a genuine equity-drawdown-overlap-timing  |
//|   analysis across symbols becomes possible. Trading logic is        |
//|   byte-identical to EA_DCA_CENT_V1.mq5 as of commit 793d30a (the     |
//|   InpMaxInitialLot-fix re-baseline) — verify with a diff against     |
//|   every non-forensic line before trusting this claim.               |
//|   DO NOT hand-edit trading logic here — port fixes to DCA_EA.mq5     |
//|   first, per the parent file's own convention below.                |
//|                                                                    |
//| FROZEN BASELINE for the cent-account go-live validation process   |
//| (see GO_LIVE_VALIDATION_PLAN.md). Re-baselined to DCA_EA.mq5 as of  |
//| commit 0964f35 (TP/SL trading mode added — TPSL_MODE_AUDIT.md /     |
//| TPSL_EQUITY_BALANCE_ROOT_CAUSE.md — including the confirmed Fix 1   |
//| mode-gated breakeven check; Fix 2 was tested, not confirmed, and    |
//| reverted upstream before this re-baseline, so it carries neither).  |
//| InpTradeMode still defaults to MODE_DCA, matching every existing    |
//| cent-account .set file, so this re-baseline is a pure superset:     |
//| existing DCA-mode backtest/optimization history for this file       |
//| remains valid unchanged; TP/SL mode is available but unexercised    |
//| on the cent-account track until deliberately opted into and         |
//| validated there. On top of that base, this file keeps its own       |
//| cent-account-only addition not present in DCA_EA.mq5: the Max        |
//| Floating Loss circuit breaker (InpUseMaxFloatingLoss /               |
//| CheckMaxFloatingLoss(), from commit 72c1f2d) — a real loss-based     |
//| backstop added here because RoboForex-specific risk/drawdown         |
//| findings (see GO_LIVE_VALIDATION_PLAN.md) don't apply to             |
//| DCA_EA.mq5's own FTMO-based development/comparison track.            |
//| Named/versioned separately per that plan's Phase 2 so this file      |
//| can be baselined, optimized and deployed toward the live cent        |
//| account on its own track, without colliding with ongoing dev/        |
//| comparison work on DCA_EA.mq5 (magic number, chart instance, and     |
//| .set files are kept independent between the two).                    |
//|                                                                      |
//| DO NOT hand-edit this file to fix bugs or add features — port the    |
//| change to DCA_EA.mq5 first, validate it there, then deliberately     |
//| re-baseline this file (new commit, bump the Vn suffix if the         |
//| change is significant) so there is always a clear record of          |
//| exactly which DCA_EA.mq5 state any given live/cent-account test      |
...
#property description "Dollar-Cost-Averaging (DCA) trade management EA — cent-account go-live baseline."
...
//+------------------------------------------------------------------+
//| Equity forensic logging (this build only, see file header).        |
//| Pure read: opens one CSV per (symbol, magic) pair and appends one   |
//| row per closed bar with the live account's Balance/Equity/floating  |
//| profit. No trading decision anywhere reads this data back — never   |
//| touches Sequence state, order flow, or SaveState()/LoadState().     |
//+------------------------------------------------------------------+
int g_eqFileHandle = INVALID_HANDLE;

void ForensicEquityInit()
  {
   string fname = "equity_forensic_" + _Symbol + "_" + IntegerToString(InpMagicNumber) + ".csv";
   g_eqFileHandle = FileOpen(fname, FILE_WRITE | FILE_CSV | FILE_ANSI, ',');
   if(g_eqFileHandle == INVALID_HANDLE)
     {
      Print("EQUITY FORENSIC: failed to open output file, error ", GetLastError());
      return;
     }
   FileWrite(g_eqFileHandle, "time", "balance", "equity", "floating_profit");
  }

void ForensicEquityLogBar()
  {
   if(g_eqFileHandle == INVALID_HANDLE) return;
   double balance = AccountInfoDouble(ACCOUNT_BALANCE);
   double equity  = AccountInfoDouble(ACCOUNT_EQUITY);
   FileWrite(g_eqFileHandle, TimeToString(TimeCurrent(), TIME_DATE | TIME_SECONDS),
             DoubleToString(balance, 2), DoubleToString(equity, 2), DoubleToString(equity - balance, 2));
   FileFlush(g_eqFileHandle);
  }
...
//--- pip size: 10x point on a 3/5-digit (fractional-pip) broker, 1x otherwise
   ...
   ForensicEquityInit();
   ForensicEquityLogBar();   // seed row at t=0 so the series has a defined starting point
