//+------------------------------------------------------------------+
//|                                              DCA_EA_Forensic.mq5 |
//|              DIAGNOSTIC BUILD — NOT the live EA, do not deploy.   |
//|                                                                    |
//| Exact copy of DCA_EA.mq5 with one addition: ProcessNewBar() writes |
//| one CSV row per closed bar (DCA_EA_Forensic_Log.csv, in the        |
//| terminal's MQL5\Files\ folder) recording the full BB/QQE/QMP        |
//| snapshot, every zone-arm/centre-cross latch, and every individual  |
//| NewSequenceGatesPass() gate's pass/fail state for BOTH directions   |
//| — not just whichever direction the bar's own dot happened to be.   |
//| Every added call is a pure read (SessionProfitLimitReached(),      |
//| ZoneArmedForEntry(), CentreCrossReady(), etc. all already exist in |
//| the live file and mutate nothing); trading logic itself is         |
//| byte-identical to DCA_EA.mq5, so a backtest run through this file  |
//| produces the exact same deal log as the live EA under the same     |
//| .set — the CSV is a parallel, non-invasive trace, not a behavior    |
//| change. Built to verify root causes for specific dates against     |
//| ground truth instead of inferring them from the deal log alone.    |
//| Delete once the investigation it was built for is done.            |
//|                                                                    |
//| Everything below this point is unchanged from DCA_EA.mq5 except    |
//| ProcessNewBar() (search "FORENSIC" for every touch point) and the  |
//| new g_forensicHandle global / file-open-close in OnInit/OnDeinit.  |
//|                                                                    |
...
//--- FORENSIC BUILD (diagnostic-only, not the live EA): logs every closed-bar's
//--- indicator snapshot and every entry-gate's individual pass/fail state to CSV,
//--- so root causes for "unexpected"/"missed" entries can be verified against
//--- ground truth instead of inferred from the deal log alone. Zero effect on
//--- trading behavior — every added call is a pure read, nothing here is used by
//--- any decision. See DCA_EA_Forensic.mq5 header for how it differs from the
//--- live EA.
int g_forensicHandle = INVALID_HANDLE;
...
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| FORENSIC ONLY — sums total open legs across every sequence in a    |
//| direction, used to detect add-ons/new-sequence opens by before/    |
//| after delta without touching any trading logic.                    |
//+------------------------------------------------------------------+
int ForensicTotalLegs(bool isBuy)
  {
   int total = 0;
   int n = isBuy ? ArraySize(g_buySequences) : ArraySize(g_sellSequences);
   for(int i = 0; i < n; i++)
      total += isBuy ? g_buySequences[i].count : g_sellSequences[i].count;
   return(total);
  }
...
void ProcessNewBar()
  {
   ...
   //--- FORENSIC: snapshot every gate's individual state exactly as this bar's
   //--- entry logic will see it, BEFORE any of it runs (ConsumeZoneLatch()/
   //--- pendingSignal writes below would otherwise contaminate this reading).
   bool f_bbBuyArmed = g_bbBuyArmed, f_bbSellArmed = g_bbSellArmed;
   bool f_qqeBuyArmed = g_qqeBuyArmed, f_qqeSellArmed = g_qqeSellArmed;
   bool f_ccReadyBuy = g_centreCrossReadyBuy, f_ccReadySell = g_centreCrossReadySell;
   int  f_prevCentreSide = g_prevCentreSide;
   long f_spreadPoints = SymbolInfoInteger(_Symbol, SYMBOL_SPREAD);
   bool f_spreadOk = SpreadOk();
   bool f_pendingIn = g_pendingSignal, f_pendingIsBuyIn = g_pendingSignalIsBuy, f_pendingBreachedIn = g_pendingSignalCentreBreached;
   int  f_buySeqBefore = ArraySize(g_buySequences), f_sellSeqBefore = ArraySize(g_sellSequences);
   int  f_buyLegsBefore = ForensicTotalLegs(true), f_sellLegsBefore = ForensicTotalLegs(false);

   bool f_buy_sessionLimitOk = !SessionProfitLimitReached();
   bool f_buy_zoneArmed      = ZoneArmedForEntry(true);
   bool f_buy_noTriggerOk    = NoTriggerOnCentreBreachOk();
   bool f_buy_ccReady        = CentreCrossReady(true);
   bool f_buy_bbWidthOk      = BBWidthOk();
   bool f_buy_reentryReady   = ReentryReady(true);
   bool f_buy_maFilterOk     = MAFilterOk(true);
   bool f_buy_htfDirOk       = HtfDirectionOk(true);
   bool f_buy_allPass        = f_buy_sessionLimitOk && f_buy_zoneArmed && f_buy_noTriggerOk && f_buy_ccReady &&
                                f_buy_bbWidthOk && f_buy_reentryReady && f_buy_maFilterOk && f_buy_htfDirOk;

   bool f_sell_sessionLimitOk = !SessionProfitLimitReached();
   bool f_sell_zoneArmed      = ZoneArmedForEntry(false);
   bool f_sell_noTriggerOk    = NoTriggerOnCentreBreachOk();
   bool f_sell_ccReady        = CentreCrossReady(false);
   bool f_sell_bbWidthOk      = BBWidthOk();
   bool f_sell_reentryReady   = ReentryReady(false);
   bool f_sell_maFilterOk     = MAFilterOk(false);
   bool f_sell_htfDirOk       = HtfDirectionOk(false);
   bool f_sell_allPass        = f_sell_sessionLimitOk && f_sell_zoneArmed && f_sell_noTriggerOk && f_sell_ccReady &&
                                 f_sell_bbWidthOk && f_sell_reentryReady && f_sell_maFilterOk && f_sell_htfDirOk;

   bool f_newSeqOpened = false;
   ...
   //--- FORENSIC: write the full row now that this bar's entry logic has finished.
   if(g_forensicHandle != INVALID_HANDLE)
     {
      int buyLegsAfter  = ForensicTotalLegs(true);
      int sellLegsAfter = ForensicTotalLegs(false);
      int addonBuyCount  = MathMax(0, buyLegsAfter  - f_buyLegsBefore  - ((f_newSeqOpened && dot > 0) ? 1 : 0));
      int addonSellCount = MathMax(0, sellLegsAfter - f_sellLegsBefore - ((f_newSeqOpened && dot < 0) ? 1 : 0));

      FileWrite(g_forensicHandle,
                TimeToString(iTime(_Symbol, PERIOD_CURRENT, 1), TIME_DATE | TIME_MINUTES | TIME_SECONDS),
                DoubleToString(g_bar1High, _Digits), DoubleToString(g_bar1Low, _Digits), DoubleToString(g_bar1Close, _Digits),
                DoubleToString(g_bbUpper1, _Digits), DoubleToString(g_bbMiddle1, _Digits), DoubleToString(g_bbLower1, _Digits), (string)g_bbSnapshotValid,
                DoubleToString(g_qqeLine1, 2), (string)g_qqeSnapshotValid,
                dot,
                (string)f_bbBuyArmed, (string)f_bbSellArmed, (string)f_qqeBuyArmed, (string)f_qqeSellArmed,
                (string)f_ccReadyBuy, (string)f_ccReadySell, f_prevCentreSide, f_spreadPoints, (string)f_spreadOk,
                (string)f_pendingIn, (string)f_pendingIsBuyIn, (string)f_pendingBreachedIn,
                f_buySeqBefore, f_sellSeqBefore,
                (string)f_buy_sessionLimitOk, (string)f_buy_zoneArmed, (string)f_buy_noTriggerOk, (string)f_buy_ccReady, (string)f_buy_bbWidthOk, (string)f_buy_reentryReady, (string)f_buy_maFilterOk, (string)f_buy_htfDirOk, (string)f_buy_allPass,
                (string)f_sell_sessionLimitOk, (string)f_sell_zoneArmed, (string)f_sell_noTriggerOk, (string)f_sell_ccReady, (string)f_sell_bbWidthOk, (string)f_sell_reentryReady, (string)f_sell_maFilterOk, (string)f_sell_htfDirOk, (string)f_sell_allPass,
                ArraySize(g_buySequences), ArraySize(g_sellSequences), (string)f_newSeqOpened, addonBuyCount, addonSellCount);
     }
  }
...
//--- pip size: 10x point on a 3/5-digit (fractional-pip) broker, 1x otherwise
   ...
   g_forensicHandle = FileOpen("DCA_EA_Forensic_Log.csv", FILE_WRITE | FILE_CSV | FILE_ANSI, ',');
   if(g_forensicHandle != INVALID_HANDLE)
      FileWrite(g_forensicHandle,
                "bar_time","bar_high","bar_low","bar_close",
                "bb_upper","bb_middle","bb_lower","bb_snapshot_valid",
                "qqe_line","qqe_snapshot_valid",
                "dot",
                "bbBuyArmed","bbSellArmed","qqeBuyArmed","qqeSellArmed",
                "centreCrossReadyBuy","centreCrossReadySell","prevCentreSide","spreadPoints","spreadOk",
                "pendingSignal_in","pendingSignalIsBuy_in","pendingSignalCentreBreached_in",
                "openBuySeq_before","openSellSeq_before",
                "buy_sessionLimitOk","buy_zoneArmed","buy_noTriggerBreachOk","buy_centreCrossReady","buy_bbWidthOk","buy_reentryReady","buy_maFilterOk","buy_htfDirOk","buy_ALLGATESPASS",
                "sell_sessionLimitOk","sell_zoneArmed","sell_noTriggerBreachOk","sell_centreCrossReady","sell_bbWidthOk","sell_reentryReady","sell_maFilterOk","sell_htfDirOk","sell_ALLGATESPASS",
                "openBuySeq_after","openSellSeq_after","newSeqOpened_this_bar","addon_buy_count","addon_sell_count");
   else
      Print("EA-DCA-FORENSIC WARNING: could not open forensic log file, error ", GetLastError());
...
void OnDeinit(const int reason)
  {
   ...
   if(g_forensicHandle != INVALID_HANDLE)
     {
      FileClose(g_forensicHandle);
      g_forensicHandle = INVALID_HANDLE;
     }
  }
